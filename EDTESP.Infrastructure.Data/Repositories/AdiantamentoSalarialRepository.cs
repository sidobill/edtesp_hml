...
{
    public class AdiantamentoSalarialRepository : IAdiantamentoSalarialRepository
    {
        public List<AdiantamentoSalarial> RetonarDados(int vendedorId, DateTime dataInicial, DateTime dataFinal, int time = 0)
        {
            using (DbContexto contexto = new DbContexto())
            {
                var query = new StringBuilder();
                query.Append("SELECT c.Numero, cl.RazaoSocial, c.DataCriacao, c.AnoEdicao, t.Descricao AS 'Time', count(1) AS 'QtdParcela', ");
                query.Append("c.ValorBase, v.Comissao, sc.StatusContratoId, sc.Descricao AS 'Status', c.ValorFinal ");
                ...
                var resultado = contexto.Database.SqlQuery<AdiantamentoSalarial>(query.ToString()).ToList();
