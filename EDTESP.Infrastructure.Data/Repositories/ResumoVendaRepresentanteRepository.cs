...
{
    public class ResumoVendaRepresentanteRepository : IResumoVendaRepresentanteRepository
    {
        public List<ResumoVendaRepresentante> RetornarDados(int vendedor, DateTime dataInicial, DateTime dataFinal, int time = 0)
        {
            using (DbContexto contexto = new DbContexto())
            {
                ...
                var resultado = contexto.Database.SqlQuery<ResumoVendaRepresentante>(query.ToString()).ToList();
